...
namespace Mitfart.LeoECSLite.Extensions.LeoECSLite.Extensions.Pool{
	public static class EcsPoolExt{
	
		public static void TryAdd<T>(this EcsPool<T> pool, int e) where T : struct{ if (!pool.Has(e)) pool.Add(e); }
		public static void TryDel<T>(this EcsPool<T> pool, int e) where T : struct{ if ( pool.Has(e)) pool.Del(e); }
		...
		public static bool TryGet<T>(this EcsPool<T> pool, int e, out T value) where T : struct{
			...
			if (world.IsAlive() && world.GetEntityGen(e) > 0 && pool.Has(e)){
				value = pool.Get(e);
				return true;
			}
